...
$viewer_id = $_SESSION['user_id'];
    if ($originator == $viewer_id) {
        $updatedelete = mysqli_query($dbc, "UPDATE pm SET `sdelete` = '1', `deletetime` = NOW() WHERE id='$pmid' LIMIT 1");
    ...
    if ($originator != $viewer_id) {
        ...
        $updatedelete = mysqli_query($dbc, "UPDATE pm SET `rdelete` = '1', `deletetime` = NOW() WHERE id='$pmid' LIMIT 1");
        $delete_perm = mysqli_query($dbc, "DELETE FROM pm  WHERE rdelete='1' AND sdelete='1' ");
    }
